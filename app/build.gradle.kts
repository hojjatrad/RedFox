...
// RedFox Xray: build libxray.aar from core/xray-mobile before the app compiles.
// The script is a no-op when the AAR is already present (e.g. committed or
// produced by an earlier CI step), so plain `gradlew assembleDebug` still works
// in environments without Go — it then links against the existing app/libs/xray.aar.
tasks.register<Exec>("buildXrayAar") {
    group = "build"
    description = "Builds the Xray-core mobile AAR into app/libs (skipped if it exists)"
    val aar = file("libs/xray.aar")
    val script = rootProject.file("core/xray-mobile/build-aar.sh")
    ...
    inputs.dir(rootProject.file("core/xray-mobile"))
    outputs.file(aar)
    onlyIf { !aar.exists() }
}
// buildXrayAar hook kept; the AAR is prebuilt and present (onlyIf skips).

// Rust core build skipped: RedFox uses the C aether stub (Xray path).
