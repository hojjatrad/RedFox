...
class MainActivity : Activity() {
    ...
    private var selectedProtocol = Protocol.XRAY
    ...
    private var xrayConfigsRow: OrbitSettingsRow? = null
     ...
     * the screen as a plain "قطع است" and hide the real reason — this flag
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        tileSpeed = MetricTile(this, palette, "سرعت", palette.amber, Sculpt.lighten(palette.amber, 0.30f)) {
            openTrafficMonitorScreen()
        }
        exitNodeCard = ExitNodeCard(this, palette) { refreshPublicIp() }
        chainCard = ChainModeCard(this, palette) { armed -> setChainArmed(armed) }
        transportRail = TransportRail(this, palette, availableProtocols.map { railLabel(it) }) { index ->
            updateConnectionMode(availableProtocols[index])
        }
        transportRail.select(availableProtocols.indexOf(selectedProtocol).coerceAtLeast(0), animate = false)
        renderChainCard()
        actionBar = OrbitActionBar(this, palette, listOf(
            OrbitActionBar.Entry("LOG", OrbitActionBar.Glyph.LOG) { openLogsScreen() },
            OrbitActionBar.Entry("تفکیک", OrbitActionBar.Glyph.SPLIT) { openSplitTunnelScreen() },
            OrbitActionBar.Entry("حالت اسکن", OrbitActionBar.Glyph.SCAN) { openScannerScreen() },
        ))
        ...
        footerWave = OrbitFooterWave(this, palette, "محافظت‌شده با RedFox")
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == VPN_REQUEST && resultCode == RESULT_OK) {
            ...
            showDisconnected("مجوز VPN لازم است")
        }
        pendingConfig = null
    }
    ...
    private fun watchForTunnelBytes(request: Int, rxAtStart: Long, rebased: Boolean = false) {
        sessionHandler.postDelayed({
            ...
            connectionDetail.text = "تونل برقرار است اما ترافیکی عبور نمی‌کند"
            renderStatusLed()
        }, BYTE_WATCH_MS)
    }
    ...
    private fun showVerifying() {
        showConnectionProgress("در حال بررسی", "در حال بررسی عبور واقعی ترافیک")
    }
    ...
    /** مسیرهایی که در این نسخه فعال‌اند (مسیرهای مبتنی بر کلودفلر حذف شده‌اند). */
    private val availableProtocols: List<Protocol>
        get() = Protocol.entries.filter { it.androidAvailable }
    ...
    private fun refreshPublicIp() {
        ...
        Thread {
            ...
            runOnUiThread {
                ...
                val (ip, country) = result.getOrElse { "IP در دسترس نیست" to "" }
                exitNodeCard.render(ip, country.takeIf { it.isNotBlank() }, isTunnelActive())
                if (isTunnelActive() && ip != "IP در دسترس نیست") {
    ...
    private fun fetchPublicIp(): Pair<String, String> {
        ...
        for (url in IP_INFO_URLS) {
            try {
                ...
                try {
                    ...
                    check(ip.isNotBlank()) { "IP در دسترس نیست" }
        ...
        throw failure ?: IllegalStateException("IP در دسترس نیست")
    }
    ...
    private fun openTunnelConnection(url: String): HttpURLConnection {
        ...
        // says "اتصال ناپایدار" while the tunnel is carrying traffic fine.
        //
    ...
    private fun openLogsScreen() {
        ...
        val header = LinearLayout(this).apply {
            ...
            addView(label("گزارش‌ها", 22f, INK, TypefaceStyle.MEDIUM), LinearLayout.LayoutParams(
                0,
                ViewGroup.LayoutParams.WRAP_CONTENT,
                1f,
            ))
            ...
            addView(createLogActionButton("کپی") { copyFullLog() })
            addView(createLogActionButton("SHARE") { shareFullLog() })
        }
        content.addView(header)
        content.addView(label("رویدادهای تونل و VPN", 14f, MUTED), LinearLayout.LayoutParams(
            ViewGroup.LayoutParams.MATCH_PARENT,
            ViewGroup.LayoutParams.WRAP_CONTENT,
    ...
    internal fun animatePageOpen(page: View) {
        page.alpha = 0f
    ...
    internal fun animatePageClose(page: View, onEnd: () -> Unit) {
        page.animate()
            .alpha(0f)
    ...
    internal fun staggerListItems(container: ViewGroup) {
    ...
    private fun connectionLogText(tab: LogTab = LogTab.ALL): String {
        ...
        return events.joinToString("\n").ifBlank { "هنوز رویداد اتصالی ثبت نشده" }
    }
    ...
    private fun copyFullLog() {
        Thread({
            ...
            runOnUiThread {
                ...
                clipboard?.setPrimaryClip(ClipData.newPlainText("گزارش RedFox", text))
                    ...
                    startActivity(
                        Intent.createChooser(
                            Intent(Intent.ACTION_SEND)
                                .setType("text/plain")
                                .putExtra(Intent.EXTRA_SUBJECT, "گزارش RedFox")
    ...
    private fun openScannerScreen(animate: Boolean = true) {
        ...
        val header = LinearLayout(this).apply {
            ...
            addView(label("گزینه‌های اسکنر", 22f, INK, TypefaceStyle.MEDIUM).apply {
        ...
        options.addView(label("حالت اسکن", 12f, MUTED).apply { letterSpacing = 0.1f }, LinearLayout.LayoutParams(
            ViewGroup.LayoutParams.MATCH_PARENT,
            ViewGroup.LayoutParams.WRAP_CONTENT,
        ).apply { topMargin = dp(20) })
        ...
        options.addView(label("نسخه‌ی IP", 12f, MUTED).apply { letterSpacing = 0.1f }, LinearLayout.LayoutParams(
            ViewGroup.LayoutParams.MATCH_PARENT,
            ViewGroup.LayoutParams.WRAP_CONTENT,
        ).apply { topMargin = dp(20) })
    ...
    private fun createScannerOption(target: ScanTarget, onSelect: (ScanTarget) -> Unit): SelectionOption {
        ...
        val indicator = label("انتخاب‌شده", 11f, primary, TypefaceStyle.MEDIUM).apply { letterSpacing = 0.08f }
    ...
    // ---------------- RedFox Xray: کانفیگ و اشتراک شخصی ----------------

    private fun xraySummary(): String {
        val active = com.msnguard.vpn.xray.XrayProfileStore.active(this)
        val count = com.msnguard.vpn.xray.XrayProfileStore.profiles(this).size
        return active?.let { "فعال: ${it.remark}" } ?: if (count > 0) "$count کانفیگ" else "هنوز کانفیگی وجود ندارد"
    }

    /** صفحات تنظیمات بعد از بسته‌شدن صفحه‌ی Xray فراخوانی می‌شوند تا ردیف‌ها تازه شوند. */
    fun refreshSettingsRows() {
        xrayConfigsRow?.setValue(xraySummary())
        connectionModeRow?.setValue(selectedProtocol.label)
    }

    private fun openXrayScreen() {
        if (visualState == OrbitDialView.State.CONNECTING ||
            visualState == OrbitDialView.State.CONNECTED ||
            TunnelStatus.isActive()
        ) return
        XrayScreen(this).open()
    }

    /** منوی انتخاب ساده (از داخل XrayScreen استفاده می‌شود). */
    fun choiceDialog(title: String, options: List<Pair<String, () -> Unit>>) {
    ...
    private fun openModeScreen() {
        ...
        val header = LinearLayout(this).apply {
            ...
            addView(label("حالت اتصال", 22f, INK, TypefaceStyle.MEDIUM).apply {
                setPadding(dp(4), 0, 0, 0)
            })
        }
        ...
        content.addView(label("انتخاب نحوه‌ی اتصال RedFox", 14f, MUTED), LinearLayout.LayoutParams(
            ViewGroup.LayoutParams.MATCH_PARENT,
            ViewGroup.LayoutParams.WRAP_CONTENT,
        ).apply { leftMargin = dp(4); bottomMargin = dp(24) })

        val options = LinearLayout(this).apply { orientation = LinearLayout.VERTICAL }
        Protocol.entries.filter { it.androidAvailable }.forEachIndexed { index, protocol ->
    ...
    private fun createModeOption(protocol: Protocol): LinearLayout {
        ...
        return LinearLayout(this).apply {
            ...
            if (selected) addView(label("فعلی", 11f, primary, TypefaceStyle.MEDIUM).apply {
                letterSpacing = 0.08f
            }) else if (!protocol.androidAvailable) addView(label("فقط رایانه", 11f, MUTED, TypefaceStyle.MEDIUM).apply {
                letterSpacing = 0.05f
            })
        }
    }
    ...
    private fun openSettingsScreen(animate: Boolean = true) {
        ...
        val header = LinearLayout(this).apply {
            ...
            addView(label("تنظیمات", 22f, INK, TypefaceStyle.MEDIUM).apply {
                setPadding(dp(4), 0, 0, 0)
            })
        }
        content.addView(sectionLabel("محافظت"), LinearLayout.LayoutParams(
            ViewGroup.LayoutParams.MATCH_PARENT,
            ViewGroup.LayoutParams.WRAP_CONTENT,
        ))
        lateinit var killSwitchRow: LinearLayout
        killSwitchRow = createToggleRow("قطع‌کننده‌ی اضطراری", "اگر تونل قطع شد، کل ترافیک مسدود شود", killSwitchEnabled()) {
        ...
        val autoReconnectRow = createToggleRow(
            "اتصال مجدد خودکار",
            "در صورت قطع تونل، خودکار وصل شود",
        ...
        content.addView(sectionLabel("مسیریابی و داده"), LinearLayout.LayoutParams(
            ViewGroup.LayoutParams.MATCH_PARENT,
            ViewGroup.LayoutParams.WRAP_CONTENT,
        ).apply { topMargin = dp(26) })
        content.addView(navRow("نمایشگر ترافیک", trafficHeadline()) { openTrafficMonitorScreen() }, LinearLayout.LayoutParams(
            ViewGroup.LayoutParams.MATCH_PARENT,
            ViewGroup.LayoutParams.WRAP_CONTENT,
        ).apply { topMargin = dp(10) })
        splitTunnelSummaryButton = navRow("تونل تفکیکی", splitTunnelSummary()) { openSplitTunnelScreen() }
        ...
        perfRow = navRow("کارایی", perfProfile().label) {
        ...
        content.addView(sectionLabel("اتصال"), LinearLayout.LayoutParams(
            ViewGroup.LayoutParams.MATCH_PARENT,
            ViewGroup.LayoutParams.WRAP_CONTENT,
        ).apply { topMargin = dp(26) })
        ...
        val modeRow = navRow("حالت اتصال", selectedProtocol.label) { openModeScreen() }
        ...
        // --- RedFox Xray: لینک اشتراک و کانفیگ‌های شخصی کاربر ---
        content.addView(sectionLabel("REDFOX XRAY"), LinearLayout.LayoutParams(
            ViewGroup.LayoutParams.MATCH_PARENT,
            ViewGroup.LayoutParams.WRAP_CONTENT,
        ).apply { topMargin = dp(26) })
        val xrayRow = navRow("کانفیگ‌ها و اشتراک", xraySummary()) { openXrayScreen() }
        xrayConfigsRow = xrayRow
        content.addView(xrayRow, LinearLayout.LayoutParams(
            ViewGroup.LayoutParams.MATCH_PARENT,
            ViewGroup.LayoutParams.WRAP_CONTENT,
        ).apply { topMargin = dp(10) })
        content.addView(navRow("کنترل‌های تونل", "شکل‌دهی ترافیک · ضد DPI") { openTunnelControlsScreen() }, LinearLayout.LayoutParams(
            ViewGroup.LayoutParams.MATCH_PARENT,
            ViewGroup.LayoutParams.WRAP_CONTENT,
        ).apply { topMargin = dp(8) })
        ...
        content.addView(sectionLabel("سایفون"), LinearLayout.LayoutParams(
            ViewGroup.LayoutParams.MATCH_PARENT,
            ViewGroup.LayoutParams.WRAP_CONTENT,
        ).apply { topMargin = dp(26) })
        ...
        val chainRow = OrbitToggleRow(
            ...
            palette,
            "Psiphon over WARP",
            "عبور سایفون از داخل یک مسیر WARP",
        ...
        val outerRow = navRow("مسیر بیرونی", chainOuterMode().label) {
        ...
        val countryRow = navRow("کشور دلخواه", egressRegionLabel()) {
        ...
        content.addView(sectionLabel("تور"), LinearLayout.LayoutParams(
            ViewGroup.LayoutParams.MATCH_PARENT,
            ViewGroup.LayoutParams.WRAP_CONTENT,
        ).apply { topMargin = dp(26) })
        val torModeRow = navRow("حالت اتصال", torMode().label) {
            chooseTorMode {
        ...
        val torOuterRow = navRow("مسیر بیرونی", torChainOuterMode().label) {
        ...
        // Same control as Psiphon's "کشور دلخواه", same wording, same
        ...
        val torCountryRow = navRow("کشور دلخواه", torRegionLabel()) {
        ...
        content.addView(sectionLabel("درباره"), LinearLayout.LayoutParams(
            ViewGroup.LayoutParams.MATCH_PARENT,
            ViewGroup.LayoutParams.WRAP_CONTENT,
        ).apply { topMargin = dp(26) })
        content.addView(navRow("بررسی به‌روزرسانی", "v${appVersion()}") {
            appUpdater.checkForUpdate()
        }, LinearLayout.LayoutParams(
            ViewGroup.LayoutParams.MATCH_PARENT,
            ViewGroup.LayoutParams.WRAP_CONTENT,
        ).apply { topMargin = dp(10) })
        content.addView(navRow("سورس در گیت‌هاب", iconRes = R.drawable.ic_github) {
            openLink("https://github.com/hojjatrad/RedFox")
        }, LinearLayout.LayoutParams(
            ViewGroup.LayoutParams.MATCH_PARENT,
            ViewGroup.LayoutParams.WRAP_CONTENT,
        ).apply { topMargin = dp(8) })
    ...
    private fun openTunnelControlsScreen() {
        ...
        content.addView(LinearLayout(this).apply {
            ...
            addView(label("کنترل‌های تونل", 22f, INK, TypefaceStyle.MEDIUM))
        })
        content.addView(label("در اتصال بعدی شما اعمال می‌شود", 13.5f, MUTED), LinearLayout.LayoutParams(
        ...
        obfRow = addControl("اختفای ترافیک", obfuscationProfile().label) {
            chooseObfuscation { obfRow.setValue(obfuscationProfile().label) }
        }
        addControl("اختفای پیشرفته", advancedObfuscationSummary()) { editAdvancedObfuscation() }
        lateinit var retryRow: OrbitSettingsRow
        retryRow = addControl("تلاش مجدد وای‌گارد", if (retryObfuscationProfiles()) "روشن" else "خاموش") {
            preferences().edit().putBoolean(RETRY_OBFUSCATION, !retryObfuscationProfiles()).apply()
            retryRow.setValue(if (retryObfuscationProfiles()) "روشن" else "خاموش")
        }
        content.addView(sectionLabel("مسیریابی"), LinearLayout.LayoutParams(
            ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.WRAP_CONTENT,
        ).apply { topMargin = dp(26) })
        addControl("نقطه‌ی پایانی دستی", manualEndpoint() ?: "خودکار") { editManualEndpoint() }
        addControl("حافظه‌ی گیت‌وی", defaultEndpointDiscovery().label) { manageGatewayCache() }
        content.addView(sectionLabel("عیب‌یابی"), LinearLayout.LayoutParams(
            ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.WRAP_CONTENT,
        ).apply { topMargin = dp(26) })
        lateinit var tlsRow: OrbitSettingsRow
        tlsRow = addControl("اثرانگشت TLS", tlsCurvePreset().label) {
            chooseTlsCurvePreset { tlsRow.setValue(tlsCurvePreset().label) }
        }
        lateinit var verificationRow: OrbitSettingsRow
        verificationRow = addControl("راستی‌آزمایی وای‌گارد", if (wireGuardDataCheck()) "سخت‌گیرانه" else "سریع") {
            preferences().edit().putBoolean(WIREGUARD_DATA_CHECK, !wireGuardDataCheck()).apply()
            verificationRow.setValue(if (wireGuardDataCheck()) "سخت‌گیرانه" else "سریع")
        }
        // The "هسته‌ی VPN" section is gone. It held DNS resolvers, Destination
        ...
        content.addView(sectionLabel("ضد فیلترینگ"), LinearLayout.LayoutParams(
            ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.WRAP_CONTENT,
        ).apply { topMargin = dp(26) })
        lateinit var fragRow: OrbitSettingsRow
        fragRow = addControl("تکه‌تکه‌سازی TLS", if (h2Fragmentation() == H2Fragmentation.ON) "روشن" else "خاموش") {
            chooseH2Fragmentation {
                fragRow.setValue(if (h2Fragmentation() == H2Fragmentation.ON) "روشن" else "خاموش")
            }
        }
        scroll.addView(content)
        page.addView(scroll)
    ...
    private fun chooseObfuscation(after: (() -> Unit)? = null) {
        ...
        sheet.addView(LinearLayout(this).apply {
            ...
            addView(label("اختفای ترافیک", 22f, INK, TypefaceStyle.MEDIUM))
        })
    ...
    private fun choosePerfProfile(after: (() -> Unit)? = null) = showChoiceSheet(
        title = "کارایی",
        subtitle = "تنظیم همزمانی و بافر اسکن متناسب با سخت‌افزار شما",
    ...
    private fun lanSharingSubtitle(): String {
        if (!lanSharingEnabled()) {
            return "اجازه‌ی استفاده‌ی دستگاه‌های دیگر از این تونل · فقط سایفون"
        }
    ...
    private fun chooseTorChainOuterMode(after: (() -> Unit)? = null) = showChoiceSheet(
        title = "مسیر بیرونی",
        subtitle = "کدام تونل WARP تور را در حالت تور‌روی‌وارپ حمل کند",
        ...
        ConnectionLog.record(
            if (chosen == ChainOuterMode.AUTO) {
                "مسیر بیرونی تور‌روی‌وارپ روی خودکار تنظیم شد"
            } else {
    ...
    private fun chooseChainOuterMode(after: (() -> Unit)? = null) = showChoiceSheet(
        title = "مسیر بیرونی",
        subtitle = "کدام تونل WARP سایفون را در حالت سایفون‌روی‌وارپ حمل کند",
        options = ChainOuterMode.entries.toList(),
     ...
     * silent fallback — the service surfaces "حالت خودکار را امتحان کنید" in its error instead.
     */
    private fun chooseTorMode(after: (() -> Unit)? = null) {
        val modes = TorManager.TorMode.entries.toList()
        showChoiceSheet(
            title = "حالت اتصال",
            subtitle = "نحوه‌ی دسترسی تور به شبکه؛ خودکار هر روش را تا موفقیت امتحان می‌کند.",
            options = modes,
            selected = torMode(),
            label = { it.label },
            description = { it.description },
            ...
            ConnectionLog.record(
                if (chosen == TorManager.TorMode.AUTO) {
                    "حالت اتصال تور روی خودکار تنظیم شد"
                } else {
    ...
    /** What the settings row shows: "خودکار" or "🇩🇪 Germany". */
    private fun torRegionLabel(): String {
        val code = torRegion()
        return if (code == TorRegions.AUTO) "خودکار" else TorRegions.label(code)
    }
    ...
    private fun chooseTorRegion(after: (() -> Unit)? = null) {
        ...
        showChoiceSheet(
            title = "کشور دلخواه",
            subtitle = "تور تلاش می‌کند از این کشور خارج شود؛ در صورت نبود، کشور دیگری استفاده می‌شود.",
            options = options,
            selected = torRegion(),
            label = { code ->
                if (code == TorRegions.AUTO) "خودکار" else TorRegions.label(code)
            },
            description = { code ->
    ...
    private fun egressRegionLabel(): String {
        ...
        return if (code == CoreConfig.EGRESS_REGION_AUTO) "خودکار" else PsiphonRegions.label(code)
    }
    ...
    private fun chooseEgressRegion(after: (() -> Unit)? = null) {
        ...
        showChoiceSheet(
            ...
            label = { code ->
                if (code == CoreConfig.EGRESS_REGION_AUTO) "خودکار" else PsiphonRegions.label(code)
            },
            description = { code ->
    ...
    private fun manageGatewayCache() = showChoiceSheet(
        title = "حافظه‌ی گیت‌وی",
        subtitle = "Control saved MASQUE gateway discovery data",
        options = listOf("کش و تازه‌سازی", "اسکن تازه در اتصال بعدی", "پاک‌کردن گیت‌وی‌های ذخیره‌شده"),
        selected = if (defaultEndpointDiscovery() == EndpointDiscovery.CACHE) "کش و تازه‌سازی" else "اسکن تازه در اتصال بعدی",
        label = { it },
        description = {
            when (it) {
                "کش و تازه‌سازی" -> "ابتدا گیت‌وی‌های ذخیره‌شده امتحان شوند"
                "اسکن تازه در اتصال بعدی" -> "Ignore saved gateways once"
                else -> "Remove saved gateway latency data"
            }
        },
        onSelected = { chosen ->
            when (chosen) {
                "کش و تازه‌سازی" -> preferences().edit().putString(ENDPOINT_DISCOVERY, EndpointDiscovery.CACHE.coreName).apply()
                "اسکن تازه در اتصال بعدی" -> preferences().edit().putString(ENDPOINT_DISCOVERY, EndpointDiscovery.FRESH.coreName).apply()
    ...
    private fun editManualEndpoint() {
        ...
        val field = EditText(this).apply {
            ...
            hint = "IP:port — برای خودکار خالی بگذارید"
            setTextColor(INK)
        ...
        sheet.addView(LinearLayout(this).apply {
            gravity = Gravity.CENTER_VERTICAL
            addView(createHeaderBackButton { dialog.dismiss() }, LinearLayout.LayoutParams(dp(48), dp(48)))
            addView(label("نقطه‌ی پایانی دستی", 22f, INK, TypefaceStyle.MEDIUM))
        })
        sheet.addView(label("آدرس IPv4 عددی یا IPv6 با براکت به‌همراه پورت. کشف خودکار را دور می‌زند.", 14f, MUTED), LinearLayout.LayoutParams(
            ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.WRAP_CONTENT,
        ).apply { leftMargin = dp(48); topMargin = dp(-4); bottomMargin = dp(20) })
        ...
        buttons.addView(createSettingsButton("پاک کردن") {
            preferences().edit().remove(MANUAL_ENDPOINT).apply()
            field.setText("")
        }, LinearLayout.LayoutParams(0, dp(52), 1f))
        buttons.addView(createSettingsButton("ذخیره") {
            ...
            if (!validEndpoint) {
                field.error = "از IP:port عددی استفاده کنید"
    ...
    private fun showTextSettingsSheet(
        ...
        sheet.addView(createSettingsButton("ذخیره", backgroundOverride = primary, textColorOverride = primaryContainer) {
    ...
    private fun editAdvancedObfuscation() {
        ...
        showTextSettingsSheet(
            ...
            listOf("تعداد بسته‌ی جعلی (JC)" to jc, "حداقل جعلی (JMIN)" to jmin, "JUNK MAX (JMAX)" to jmax, "INIT PACKET I1" to i1, "INIT PACKET I2" to i2),
            validator = { values ->
                val numbers = values.take(3).map { it.toIntOrNull() }
                when {
                    values.take(3).withIndex().any { (index, value) -> value.isNotBlank() && numbers[index] == null } -> 0 to "فقط عدد صحیح وارد کنید"
                    numbers[0]?.let { it !in 0..10 } == true -> 0 to "مقدار Jc باید بین ۰ تا ۱۰ باشد"
                    numbers[1]?.let { it !in 0..1024 } == true -> 1 to "Jmin must be 0–1024"
                    numbers[2]?.let { it !in 0..1024 } == true -> 2 to "مقدار Jmax باید بین ۰ تا ۱۰۲۴ باشد"
                    numbers[1] != null && numbers[2] != null && numbers[2]!! < numbers[1]!! -> 2 to "Jmax must be at least Jmin"
                    values.drop(3).any { it.length > 2048 } -> 3 to "الگوی بسته بیش از حد طولانی است"
                    else -> null
                }
            },
        ) { values ->
    ...
    private fun refreshPsiphonRows() {
        ...
        // conditions are "تور انتخاب شده است" and "در میانه‌ی نشست قفل نمی‌شود".
    ...
    private fun openTrafficMonitorScreen() {
        ...
        content.addView(LinearLayout(this).apply {
            ...
            addView(label("نمایشگر ترافیک", 22f, INK, TypefaceStyle.MEDIUM).apply { setPadding(dp(4), 0, 0, 0) })
        })
        ...
        trafficSpeedValue = addTrafficMetric(content, "سرعت لحظه‌ای")
        trafficSessionValue = addTrafficMetric(content, "این نشست")
        trafficMonthValue = addTrafficMetric(content, "این ماه")
        scroll.addView(content)
        page.addView(scroll, FrameLayout.LayoutParams(
            ViewGroup.LayoutParams.MATCH_PARENT,
            ViewGroup.LayoutParams.MATCH_PARENT,
    ...
    private fun addTrafficMetric(parent: LinearLayout, title: String): TextView {
        val value = label("در انتظار ترافیک تونل", 18f, INK, TypefaceStyle.MEDIUM)
    ...
    private fun trafficHeadline(): String =
        if (trafficMonthRx + trafficMonthTx == 0L) "هنوز داده‌ای رد نشده"
    ...
    private fun openSplitTunnelScreen() {
        ...
        val header = LinearLayout(this).apply {
            ...
            addView(label("تونل تفکیکی", 22f, INK, TypefaceStyle.MEDIUM), LinearLayout.LayoutParams(0, ViewGroup.LayoutParams.WRAP_CONTENT, 1f))
            addView(createSettingsButton("انجام شد", backgroundOverride = primary, textColorOverride = primaryContainer) {
                settings.save(settings.mode(), selected)
                closeSplitTunnelScreen()
            }, LinearLayout.LayoutParams(dp(88), dp(40)).apply { marginEnd = dp(4) })
        }
        content.addView(header)
        content.addView(label("انتخاب کنید کدام برنامه‌ها از RedFox استفاده کنند. تغییرات در اتصال بعدی اعمال می‌شوند.", 14f, MUTED), LinearLayout.LayoutParams(
            ViewGroup.LayoutParams.MATCH_PARENT,
            ViewGroup.LayoutParams.WRAP_CONTENT,
        ).apply { leftMargin = dp(48); topMargin = dp(-8); bottomMargin = dp(20) })
        content.addView(label("حالت", 12f, MUTED).apply { letterSpacing = 0.1f })
    ...
    private fun openSplitTunnelAppsScreen(mode: SplitTunnelSettings.Mode, selected: MutableSet<String>) {
        ...
        content.addView(LinearLayout(this).apply {
            ...
            addView(label("برنامه‌ها", 22f, INK, TypefaceStyle.MEDIUM), LinearLayout.LayoutParams(0, ViewGroup.LayoutParams.WRAP_CONTENT, 1f))
            addView(createSettingsButton("انجام شد", backgroundOverride = primary, textColorOverride = primaryContainer) {
        ...
        content.addView(label("Select apps to ${if (mode == SplitTunnelSettings.Mode.INCLUDE) "شامل" else "exclude"}", 14f, MUTED), LinearLayout.LayoutParams(
            ViewGroup.LayoutParams.MATCH_PARENT,
            ViewGroup.LayoutParams.WRAP_CONTENT,
        ).apply { leftMargin = dp(48); topMargin = dp(-8); bottomMargin = dp(16) })

        val searchField = EditText(this).apply {
            hint = "جستجوی برنامه‌ها…"
            setHintTextColor(MUTED)
            setTextColor(INK)
            textSize = 15f
            setSingleLine(true)
        ...
        val loading = LinearLayout(this).apply {
            ...
            addView(label("در حال بررسی برنامه‌های نصب‌شده…", 14f, MUTED).apply {
                gravity = Gravity.CENTER
    ...
    private fun updateConnectionMode(protocol: Protocol) {
        ...
        transportRail.select(availableProtocols.indexOf(protocol).coerceAtLeast(0), animate = true)
    ...
    private fun toggleTunnel() {
        ...
        // the UI sat on "در حال اتصال" until the tunnel came up on its own or the
        ...
        if (TunnelStatus.isActive() || visualState == OrbitDialView.State.CONNECTING) {
            ...
            showDisconnected("در حال قطع‌کردن")
            return
        }
    ...
    private fun renderStatus() {
        if (!TunnelStatus.isActive() && isTunnelActive()) {
            NativeCore.lastError().takeIf(String::isNotBlank)?.let(::showFailure) ?: showDisconnected("تونل به‌طور غیرمنتظره متوقف شد")
        }
    }
    ...
    private fun showConnecting(detail: String? = null, progress: Int = -1) {
        ...
        showConnectionProgress("در حال اتصال", detail ?: "Starting ${selectedProtocol.label} tunnel")
    }
    ...
    private fun showStarting() {
        ...
        showConnectionProgress("در حال شروع", "Preparing ${selectedProtocol.label} tunnel")
    }

    private fun showScanning() {
        orbitDial.progressPercent = -1
        showConnectionProgress("در حال اسکن", "Finding the best MASQUE gateway")
    }
    ...
    private fun showDegraded() {
        ...
        connectionTitle.text = "اتصال ناپایدار"
        connectionDetail.text = "Tunnel is active; HTTP health check failed"
        footerWave.setLit(false)
        chipLatency.text = "تأخیر ناموجود"
    }
    ...
    private fun showFailure(detail: String? = null) {
        ...
        footerWave.setLit(false)
        connectionDetail.text = detail ?: "سرور را بررسی کنید و دوباره تلاش کنید"
        setModeEnabled(true)
    }

    private fun showDisconnected(detail: String = "برای اتصال روی دایره بزنید") {
        latencyRequest++
        cancelVerification()
        clearCoreExitIp()
        stopAutoPing()
    ...
    private fun renderChainCard() {
        ...
        val reason = when {
            ...
            !modeControlsEnabled -> "برای تغییر، اتصال را قطع کنید"
            else -> null
        }
        // Which transport is being wrapped, so the card cannot read
        // "PSIPHON OVER WARP" while the rail has Tor selected.
        chainCard.setInner(if (selectedProtocol == Protocol.TOR) "تور" else "سایفون")
        // Two separate facts, and collapsing them was a bug: "locked" is not the
        // same as "اعمال نمی‌شود". Connected on Psiphon is locked but fully
        ...
        // is invisible here and the card would still read "مسیر خودکار" while the
    ...
    internal fun createHeaderBackButton(onClick: () -> Unit): ImageView = ImageView(this).apply {
    ...
    internal fun roundedBackground(fill: Int, radius: Int, stroke: Int, strokeWidth: Int = 1): GradientDrawable =
    ...
    private fun appVersion(): String =
        packageManager.getPackageInfo(packageName, 0).versionName ?: "نامشخص"

    internal fun createSettingsButton(
        text: String,
        icon: Int? = null,
    ...
    private fun advancedObfuscationSummary(): String =
        listOf(OBFUSCATION_JC, OBFUSCATION_JMIN, OBFUSCATION_JMAX, OBFUSCATION_I1, OBFUSCATION_I2)
            ...
            .let { if (it) "سفارشی" else "پیش‌تنظیم" }
    ...
    private fun savedProtocol(): Protocol {
        val name = preferences().getString(DEFAULT_PROTOCOL, Protocol.XRAY.coreName)
        val match = Protocol.entries.firstOrNull { it.coreName == name && it.androidAvailable }
        // مقادیر به‌جامانده از نسخه‌های مبتنی بر کلودفلر (masque/wireguard/gool)
        // دیگر مسیر فعالی ندارند؛ به Xray نگاشت می‌شوند.
        return match ?: Protocol.XRAY
    }
    ...
    internal fun sectionLabel(text: String): View = OrbitSectionHeader(this, palette, text)
    ...
    private fun splitTunnelSummary(): String {
        ...
        return when (settings.mode()) {
            SplitTunnelSettings.Mode.ALL -> "همه‌ی برنامه‌ها از RedFox استفاده می‌کنند"
    ...
    private enum class Protocol(
        ...
        XRAY("RedFox Xray", "xray", "هسته‌ی Xray — اتصال با کانفیگ یا اشتراک شخصی شما"),
        PSIPHON("Psiphon", "psiphon", "تونل ضد سانسور"),
        TOR("Tor", "tor", "مسیریابی پیازی؛ کندتر ولی سخت‌ترین برای مسدودسازی"),
        // نگهداری‌شده فقط برای سازگاری با مقادیر ذخیره‌شده‌ی نسخه‌های قبلی؛
        // در انتخابگر حالت نمایش داده نمی‌شوند و به Xray نگاشت می‌شوند.
        MASQUE("MASQUE", "masque", "تونل HTTP/3", androidAvailable = false),
        WIREGUARD("WireGuard", "wireguard", "تونل وای‌گارد", androidAvailable = false),
        WARP_IN_WARP("WARP-on-WARP", "gool", "تونل دولایه", androidAvailable = false),
    }
    ...
    private enum class ScanMode(
        ...
        TURBO("توربو", "turbo", "سریع‌ترین اسکن؛ اولین مسیر تأییدشده برنده می‌شود"),
        BALANCED("متعادل", "balanced", "ترکیب پیش‌فرض سرعت و پوشش"),
        THOROUGH("دقیق", "thorough", "اسکن عمیق؛ بهترین تأخیر انتخاب می‌شود"),
        STEALTH("پنهان‌کار", "stealth", "کاوش آرام و صبورانه"),
        IRONCLAD("Ironclad", "ironclad", "راستی‌آزمایی سخت‌گیرانه‌ی CONNECT-IP پیش از انتخاب"),
    }
    ...
    private enum class EndpointDiscovery(
        ...
        CACHE("کش و تازه‌سازی", "cache", "ابتدا گیت‌وی‌های تأییدشده، سپس کشف موارد بیشتر"),
        FRESH("Fresh scan", "fresh", "هر اتصال یک اسکن تازه"),
    }

    private enum class ObfuscationProfile(val label: String, val coreName: String, val description: String) {
        OFF("خاموش", "off", "No traffic-shape padding"),
        LIGHT("سبک", "light", "سربار کمتر در فیلترینگ ملایم"),
        BALANCED("متعادل", "balanced", "مقاومت توصیه‌شده در برابر فیلترینگ"),
        AGGRESSIVE("تهاجمی", "aggressive", "Highest resistance; slower setup"),
    }

    private enum class TlsCurvePreset(val label: String, val coreName: String, val description: String) {
        CHROME("کروم", "chrome", "Chrome TLS curve ordering"),
        COMPATIBILITY("سازگاری", "compatibility", "P-256 and X25519 only"),
    }

    private enum class LogLevel(val label: String, val coreName: String, val description: String) {
        ERROR("خطا", "error", "Only errors"),
        WARN("هشدار", "warn", "هشدارها و خطاها"),
        INFO("Info", "info", "میزان پیش‌فرض جزئیات"),
        DEBUG("Debug", "debug", "درون تونل"),
        TRACE("Trace", "trace", "جزئیات کامل هر بسته"),
    }

    private enum class PerfProfile(val label: String, val coreName: String, val description: String) {
        AUTO("خودکار", "auto", "تشخیص خودکار سخت‌افزار و تنظیم متناسب"),
        LOW("Low", "low", "روترها و دستگاه‌های محدود"),
        MEDIUM("متوسط", "medium", "سخت‌افزار متوسط"),
        HIGH("بالا", "high", "رایانه و دستگاه‌های قدرتمند"),
    }

    private enum class H2Fragmentation(val label: String, val coreName: String, val description: String) {
        ON("روشن", "on", "Fragment TLS handshake to evade DPI"),
        OFF("خاموش", "off", "Standard TLS handshake"),
    }
    ...
    private companion object {
         ...
         * endpoint having a bad day cannot paint "اتصال ناپایدار" over a
         * working tunnel.
         */
        ...
        /** Matches `"کشور":"IR"` and `"country_code":"IR"` alike. */
