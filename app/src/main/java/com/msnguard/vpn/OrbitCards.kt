...
class ExitNodeCard(
    ...
    init {
        ...
        keyView = text("گره خروج", 8.5f, Sculpt.withAlpha(palette.faint, 0.95f), medium = true, spacing = 0.14f)
        ipView = text("not tunnelled", 15f, palette.ink, medium = true, mono = true).apply {
            setSingleLine(true)
            ellipsize = TextUtils.TruncateAt.END
        }
        locView = text("برای تازه‌سازی بزنید", 10.5f, Sculpt.withAlpha(palette.faint, 0.9f))
    ...
    fun render(
        ...
        keyView.text = if (tunnelled) "گره خروج" else "IP شما"
        if (address.isBlank() || address == UNAVAILABLE) {
            ...
            if (measuring) {
                ...
                flagView.text = "\uD83C\uDF10"
                contentDescription = "در حال اندازه‌گیری آدرس خروجی تونل"
                return
            }
            ipView.text = UNAVAILABLE
            ipView.textSize = 15f
            locView.text = "برای تلاش مجدد بزنید"
            flagView.text = "\uD83C\uDF10"
            contentDescription = "IP در دسترس نیست، برای تلاش مجدد بزنید"
            return
        }
    ...
    private companion object {
        const val UNAVAILABLE = "IP در دسترس نیست"
 ...
 *
 * Dimmed and outlined when off, lit with the violet accent and a "زنجیره‌ای" badge
 * when armed.
 */
class ChainModeCard(
    context: Context,
    ...
    private var innerName: String = "سایفون"

    /**
     * Whether the chain can apply to the selected transport at all.
     *
     * Separate from [unavailableReason] because "الان قابل تغییر نیست" and
     * "برای این مسیر اعمال نمی‌شود" are different facts and were being
     ...
     */
    private var outerSummary: String = "مسیر خودکار"
     ...
     *   old meaning. Pass true with a non-null reason for "اعمال می‌شود، اما قفل است" —
    ...
    /**
     * Names the transport this card wraps, e.g. "سایفون" or "تور".
     *
    ...
    /**
     * Says how the outer transport is chosen, e.g. "مسیر خودکار" or "از طریق WoW".
     *
    ...
    fun setArmed(value: Boolean) {
        ...
        // can be the faster of the two. "کندتر" was true of the lab and wrong in
        // the field, so the label states the effect that always holds.
        subtitleView.text = unavailableReason ?: if (value) {
            armedSubtitle()
        } else {
            "برای زمانی که هیچ IP خروجی پذیرفته نمی‌شود"
        }
        subtitleView.setTextColor(Sculpt.withAlpha(palette.faint, 0.95f))
        badgeView.text = when {
            // N/A means "برای این مسیر اعمال نمی‌شود", so it must not appear
