...
class MsnGuardTileService : TileService() {
    ...
    private fun configJson(): String {
        ...
        val picked = prefs.getString(DEFAULT_PROTOCOL, Protocol.XRAY.coreName)
    ...
    private val selectedProtocolcoreName: String
        get() = getSharedPreferences(SETTINGS, MODE_PRIVATE)
            .getString(DEFAULT_PROTOCOL, Protocol.XRAY.coreName)
            ?.let { name -> Protocol.entries.find { it.coreName == name } }
            ?.coreName ?: Protocol.XRAY.coreName
    ...
    companion object {
        ...
        enum class Protocol(
            ...
            XRAY("RedFox Xray", "xray", "هسته‌ی Xray با کانفیگ شخصی شما"),
            PSIPHON("Psiphon", "psiphon", "تونل ضد سانسور"),
            TOR("Tor", "tor", "مسیریابی پیازی"),
        }
